...
void Init_Port1(void){  // This function will initiate port 1
  P1OUT = GO_ZERO; // set all GPIO pins LOW
  P1DIR = GO_ONE; // Set all GPIO pins to input
  ...
  P1SEL0 |= V_BAT;    // set GPIO operation on bit 0
  P1SEL1 |= V_BAT;    // set GPIO operation on bit 1 
...
*/
  ...
  P2SEL0 &= ~IOT_RUN; // IOT_PROG_SEL GPIO operation
  P2SEL1 &= ~IOT_RUN; // IOT_PROG_SEL GPIO operation
...
void Init_Port3(unsigned int pin_state){  // This function will initiate port 3
  ...
  */
  ...
  P3SEL0 &= ~IOT_EN;        // set GPIO operation on bit 0
  P3SEL1 &= ~IOT_EN;        // set GPIO operation on bit 1
  P3OUT |= IOT_EN;         // set output LOW
  P3DIR |= IOT_EN;          // set pin to output
}
...
void Init_Port5(void){  // This function will initiate port 5
  ...
  P5SEL0 &= ~CHECK_BATT; // set GPIO operation on bit 0
  P5SEL1 &= ~CHECK_BATT; // set GPIO operation on bit 1
  P5OUT &= ~CHECK_BATT; // Set output as low
  P5DIR |= CHECK_BATT; // Set to output
  
  P5SEL0 &= ~V_BATT; // set GPIO operation on bit 0
  P5SEL1 &= ~V_BATT; // set GPIO operation on bit 1
  P5OUT &= ~V_BATT; // Set output as low
  P5DIR |= V_BATT; // Set to output
  
  P5SEL0 &= ~V_DAC; // set GPIO operation on bit 0
  P5SEL1 &= ~V_DAC; // set GPIO operation on bit 1
  P5OUT &= ~V_DAC; // Set output as low
  P5DIR |= V_DAC; // Set to output
  
  P5SEL0 &= ~v_3_3; // set GPIO operation on bit 0
  P5SEL1 &= ~v_3_3; // set GPIO operation on bit 1
  P5OUT &= ~v_3_3; // Set output as low
  P5DIR |= v_3_3; // Set to output
  
  /*
  P5SEL0 &= ~IOT_PROG_MODE; // set GPIO operation on bit 0
  P5SEL1 &= ~IOT_PROG_MODE; // set GPIO operation on bit 1
  P5OUT &= ~IOT_PROG_MODE; // Set output as low
  P5DIR |= IOT_PROG_MODE; // Set to output
  */
  
  P5SEL0 &= ~IOT_BOOT; // set GPIO operation on bit 0
  P5SEL1 &= ~IOT_BOOT; // set GPIO operation on bit 1
  P5OUT |= IOT_BOOT; // Set output as low
  P5DIR |= IOT_BOOT; // Set to output
}
